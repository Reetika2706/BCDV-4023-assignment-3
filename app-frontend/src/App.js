...
function App() {
  ...
  const { pairs } = item;
  const contractAddress = "0xa5e234DCe55c524461252792A42121EA68DeB675";
  const ABI =  [
    {
      "inputs": [],
  ...
  const getPair = async (pair) => {
    ...
    if (pair == "BTC/USD") {
      contractPrice =
        await smartContract?.get_BTC_USD_value();
      setStoredPrice("$" + parseInt(contractPrice) / 100000000);
    } else if (pair === "ETH/USD") {
      contractPrice =
        await smartContract.get_ETH_USD_value();
      setStoredPrice("$" + parseInt(contractPrice) / 100000000);
    } else if (pair === "LINK/USD") {
      contractPrice =
        await smartContract.get_LINK_USD_value();
      setStoredPrice("$" + parseInt(contractPrice) / 100000000);
    } else if (pair === "BTC/ETH") {
      contractPrice =
        await smartContract.get_BTC_ETH_value();
